...
import {
  ...
  ShieldAlert, X, Upload, RefreshCw, AlertCircle
} from "lucide-react";
import {
...
const STATUS_COLORS: Record<string, string> = {
  ...
  processed: "bg-purple-500/15 text-purple-500",
...
// Next allowed statuses per current status (returns flow)
const RETURN_NEXT: Record<string, Array<{ status: string; label: string; color: string }>> = {
  pending:   [
    { status: "received",  label: "Mark received",  color: "bg-blue-600/15 text-blue-600 hover:bg-blue-600/25" },
    { status: "rejected",  label: "Reject",         color: "bg-red-600/15 text-red-500 hover:bg-red-600/25" },
  ],
  received:  [
    { status: "processed", label: "Mark processed", color: "bg-purple-600/15 text-purple-600 hover:bg-purple-600/25" },
    { status: "rejected",  label: "Reject",         color: "bg-red-600/15 text-red-500 hover:bg-red-600/25" },
  ],
  processed: [
    { status: "refunded",  label: "Issue refund",   color: "bg-emerald-600/15 text-emerald-600 hover:bg-emerald-600/25" },
    { status: "rejected",  label: "Reject",         color: "bg-red-600/15 text-red-500 hover:bg-red-600/25" },
  ],
};

// Next allowed statuses per current status (refunds flow)
const REFUND_NEXT: Record<string, Array<{ status: string; label: string; color: string }>> = {
  pending: [
    { status: "refunded", label: "Mark refunded", color: "bg-emerald-600/15 text-emerald-600 hover:bg-emerald-600/25" },
    { status: "rejected", label: "Reject",         color: "bg-red-600/15 text-red-500 hover:bg-red-600/25" },
  ],
  open: [
    { status: "refunded", label: "Mark refunded", color: "bg-emerald-600/15 text-emerald-600 hover:bg-emerald-600/25" },
...
type ReturnRow = {
  ...
  reason_category: string; event_type: string; source_platform: string;
  country: string; tracking_number: string;
};
type RefundRow = {
...
export default function ReturnsPage() {
  ...
  async function updateReturn(id: string, status: string, trackingNumber?: string) {
  ...
  const pendingRefunds = refunds.filter(r => r.status === "pending" || r.status === "open");
    ...
    <div className="min-h-screen bg-[#f6f7f9] text-gray-900">
      ...
      <div className="sticky top-0 z-20 bg-[#f6f7f9]/90 backdrop-blur-md border-b border-black/5 px-8 py-4">
        <div className="max-w-7xl mx-auto flex items-center gap-4">
          ...
          <div className="flex-1">
            <h1 className="text-lg font-black leading-none">Returns & Refunds</h1>
            <p className="text-xs text-gray-400 mt-0.5">Returns, refunds and chargebacks</p>
          </div>
        ...
        {tab === "overview" && (
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-5 gap-4">
              <KpiCard label="Returns" value={t?.returns ?? 0} sub={`€${(t?.refund_total ?? 0).toFixed(0)} refunded`} color="blue" />
              <KpiCard label="Pending refunds" value={t?.dispute_open ?? 0} sub={`€${(t?.dispute_at_risk ?? 0).toFixed(0)} to process`} color="amber" />
            ...
            {(t?.pending ?? 0) > 0 && (
              <div className="rounded-2xl bg-white border border-black/8 p-5">
                <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold mb-4">Returns — pending action ({t?.pending})</p>
                  ...
                  {returns.filter(r => r.status === "pending").slice(0, 5).map(r => (
                    <div key={r.id} className="flex items-center gap-3 p-3 rounded-xl bg-amber-50 border border-amber-100">
                      ...
                      <div className="flex gap-1 shrink-0">
                        {(RETURN_NEXT[r.status] ?? []).map(a => (
                          <button key={a.status} onClick={() => updateReturn(r.id, a.status)}
                            className={`text-[10px] px-2 py-1 rounded-lg transition font-semibold ${a.color}`}>
                            {a.label}
                          </button>
                        ))}
                      </div>
            ...
            {pendingRefunds.length > 0 && (
              <div className="rounded-2xl bg-white border border-black/8 p-5">
                <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold mb-4">Refunds — to be processed ({pendingRefunds.length})</p>
                <div className="space-y-2">
                  {pendingRefunds.slice(0, 5).map(r => (
                    <div key={r.id} className="flex items-center gap-3 p-3 rounded-xl bg-orange-50 border border-orange-100">
                      <ShieldAlert size={14} className="text-orange-500 shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-sm font-medium truncate">{r.customer_email || r.order_id}</p>
                        <p className="text-xs text-gray-500">{r.order_id} · {r.reason || "—"}</p>
                      </div>
                      <p className="text-sm font-semibold shrink-0 text-red-500">€{r.amount.toFixed(2)}</p>
                      <div className="flex gap-1 shrink-0">
                        {(REFUND_NEXT[r.status] ?? []).map(a => (
                          <button key={a.status} onClick={() => updateRefund(r.id, a.status)}
                            className={`text-[10px] px-2 py-1 rounded-lg transition font-semibold ${a.color}`}>
                            {a.label}
                          </button>
                        ))}
        ...
        {tab === "events" && (
          <div className="space-y-4">
            ...
            <div className="rounded-2xl bg-white border border-black/8 overflow-hidden">
              <div className="px-5 py-4 border-b border-black/5 flex items-center justify-between">
                <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold flex items-center gap-1.5">
                  <RotateCcw size={11} /> Returns ({returns.length})
                </p>
              </div>
              {returns.length === 0 ? (
                <div className="p-10 text-center text-sm text-gray-400">No returns yet</div>
              ) : (
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-black/5">
                      {["Order", "Product", "Track & Trace", "Reason", "Category", "Amount", "Status", "Date", "Action"].map(h => (
                    ...
                    {returns.map(r => (
                      <tr key={r.id} onClick={() => setSelectedReturn(r)}
                        ...
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                          <TrackingCell value={r.tracking_number} onSave={val => updateReturn(r.id, r.status, val)} />
                        </td>
                        <td className="px-4 py-3 text-xs text-gray-500 max-w-[120px] line-clamp-2">{r.reason || "—"}</td>
                        ...
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                          <div className="flex gap-1 flex-wrap">
                            {(RETURN_NEXT[r.status] ?? []).map(a => (
                              <button key={a.status} onClick={() => updateReturn(r.id, a.status)}
                                className={`text-[10px] px-2 py-1 rounded-lg transition font-semibold whitespace-nowrap ${a.color}`}>
                                {a.label}
                              </button>
                            ))}
                          </div>
            ...
            <div className="rounded-2xl bg-white border border-black/8 overflow-hidden">
              <div className="px-5 py-4 border-b border-black/5">
                <p className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold flex items-center gap-1.5">
                  <ShieldAlert size={11} /> Refunds & Chargebacks ({refunds.length})
                </p>
              </div>
              {refunds.length === 0 ? (
                <div className="p-10 text-center text-sm text-gray-400">No refunds yet</div>
              ) : (
                <table className="w-full text-sm">
                  <thead>
                    <tr className="border-b border-black/5">
                  ...
                  <tbody>
                    {refunds.map(d => (
                      <tr key={d.id} onClick={() => setSelectedRefund(d)}
                        className="border-b border-black/5 hover:bg-black/[0.02] transition cursor-pointer">
                        ...
                        <td className="px-4 py-3">
                          <span className={`text-[10px] px-2 py-0.5 rounded-full font-semibold ${d.dispute_type === "chargeback" ? "bg-red-100 text-red-600" : "bg-amber-100 text-amber-600"}`}>
                            {d.dispute_type === "chargeback" ? "chargeback" : "refund"}
                          </span>
                        </td>
                        ...
                        <td className="px-4 py-3" onClick={e => e.stopPropagation()}>
                          <div className="flex gap-1 flex-wrap">
                            {(REFUND_NEXT[d.status] ?? []).map(a => (
                              <button key={a.status} onClick={() => updateRefund(d.id, a.status)}
                                className={`text-[10px] px-2 py-1 rounded-lg transition font-semibold whitespace-nowrap ${a.color}`}>
                                {a.label}
                              </button>
                            ))}
      ...
      {selectedReturn && (
        <ReturnDetailModal
          ...
          onUpdate={(status, tracking) => { updateReturn(selectedReturn.id, status, tracking); setSelectedReturn(null); }}
        />
      )}

      {selectedRefund && (
        <DetailModal title="Refund detail" icon={<ShieldAlert size={16} className="text-red-500" />} onClose={() => setSelectedRefund(null)}>
          <Fields items={[
            { label: "Order ID", value: selectedRefund.order_id },
            { label: "Customer", value: selectedRefund.customer_email || "—" },
            { label: "Reason", value: selectedRefund.reason || "—" },
            { label: "Category", value: selectedRefund.reason_category || "—" },
            { label: "Type", value: selectedRefund.dispute_type === "chargeback" ? "Chargeback" : "Refund" },
            { label: "Amount", value: `€${selectedRefund.amount.toFixed(2)}` },
            { label: "Status", value: selectedRefund.status },
            { label: "Recommendation", value: ACTION_SUGGESTIONS[selectedRefund.reason_category] ?? "—" },
            { label: "Date", value: selectedRefund.created_at?.slice(0, 10) },
          ]} />
          {(REFUND_NEXT[selectedRefund.status] ?? []).length > 0 && (
            <div className="flex gap-2 mt-5">
              {(REFUND_NEXT[selectedRefund.status] ?? []).map(a => (
                <button key={a.status} onClick={() => { updateRefund(selectedRefund.id, a.status); setSelectedRefund(null); }}
                  className={`flex-1 h-10 rounded-xl font-semibold text-sm transition ${a.color}`}>
                  {a.label}
                </button>
              ))}
            </div>
...
function TrackingCell({ value, onSave }: { value: string; onSave: (v: string) => void }) {
    ...
    return (
      <div className="flex items-center gap-1" onClick={e => e.stopPropagation()}>
        <input autoFocus value={val} onChange={e => setVal(e.target.value)}
          ...
          className="w-28 text-xs border border-blue-400 rounded px-1.5 py-0.5 outline-none" />
        <button onClick={() => { onSave(val); setEditing(false); }} className="text-[10px] text-emerald-600 font-bold">✓</button>
        <button onClick={() => setEditing(false)} className="text-[10px] text-gray-400 font-bold">✕</button>
      </div>
    );
  }
  ...
  return (
    <button onClick={() => { setVal(value ?? ""); setEditing(true); }}
      className="text-xs text-gray-400 hover:text-blue-600 transition font-mono underline-offset-2 hover:underline">
      {value || "+ add"}
    </button>
  );
}

function ReturnDetailModal({ r, onClose, onUpdate }: {
  r: ReturnRow;
  onClose: () => void;
  onUpdate: (status: string, tracking?: string) => void;
}) {
  const [tracking, setTracking] = useState(r.tracking_number ?? "");

  return (
    <DetailModal title="Return detail" icon={<RotateCcw size={16} className="text-blue-500" />} onClose={onClose}>
      <Fields items={[
        { label: "Order ID", value: r.order_id },
        { label: "Product", value: r.product_title },
        { label: "Variant", value: r.variant_title || "—" },
        { label: "Reason", value: r.reason || "—" },
        { label: "Category", value: r.reason_category || "—" },
        { label: "Amount", value: `€${r.amount.toFixed(2)}` },
        { label: "Status", value: r.status },
        { label: "Recommendation", value: ACTION_SUGGESTIONS[r.reason_category] ?? "—" },
        { label: "Date", value: r.created_at?.slice(0, 10) },
      ]} />

      {/* Track & Trace */}
      <div className="mt-4">
        <label className="text-[10px] text-gray-400 uppercase tracking-widest mb-1 block">Track & Trace</label>
        <input value={tracking} onChange={e => setTracking(e.target.value)}
          placeholder="e.g. 3SXXX123456789"
      ...
      {(RETURN_NEXT[r.status] ?? []).length > 0 && (
        <div className="flex gap-2 mt-4">
          {(RETURN_NEXT[r.status] ?? []).map(a => (
            <button key={a.status} onClick={() => onUpdate(a.status, tracking)}
              className={`flex-1 h-10 rounded-xl font-semibold text-sm transition ${a.color}`}>
              {a.label}
            </button>
          ))}
...
function AddEventModal({ storeId, onClose }: { storeId: string; onClose: () => void }) {
  ...
  const [form, setForm] = useState({ order_id: "", product_title: "", variant_title: "", customer_email: "", reason: "", amount: "", tracking_number: "" });
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white border border-black/10 rounded-3xl p-6 w-full max-w-md">
        ...
        <div className="flex gap-1 bg-black/5 rounded-xl p-1 mb-4">
          {(["return", "refund", "chargeback"] as const).map(t => (
        ...
        <div className="space-y-3">
          {[
            ...
            { key: "tracking_number", label: "Track & Trace",   show: type === "return" },
    ...
    try {
      ...
      const rows = lines.slice(1).map(line => {
        ...
        return {
          ...
          status: obj["status"] ?? "",
          tracking_number: obj["tracking_number"] ?? obj["tracking"] ?? "",
        };
      });
    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50">
      <div className="bg-white border border-black/10 rounded-3xl p-6 w-full max-w-lg">
        ...
        <p className="text-xs text-gray-500 mb-3">
          Columns: <code className="bg-gray-100 px-1 rounded">order_id, product_title, customer_email, reason, amount, event_type, date, tracking_number, country, source_platform, status</code>
          <br />event_type: <code className="bg-gray-100 px-1 rounded">return</code> / <code className="bg-gray-100 px-1 rounded">refund</code> / <code className="bg-gray-100 px-1 rounded">chargeback</code>
        </p>
        <textarea
          value={csv}
          onChange={e => setCsv(e.target.value)}
          placeholder={"order_id,product_title,reason,amount,event_type,date\n#1234,Robe Maxi,Size too large,34.95,return,2026-07-10"}
          rows={8}
