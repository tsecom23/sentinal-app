    ...
    <div className="px-8 py-7 min-h-screen max-w-[1400px]">
      ...
      <div className="grid grid-cols-3 gap-4 mb-4">
        <KpiCard
          ...
          value={`€${fmt(revenue)}`}
          sub={returnAmount > 0
            ? `Gross €${fmt(grossRevenue)} − €${fmt(returnAmount)} returns`
            : `AOV €${fmt(aov)}`}
          accent="blue"
          loading={loading}
        />
        <KpiCard
          label="Net Profit"
          value={`€${fmt(profit)}`}
          sub={`${margin.toFixed(1)}% margin · €${orders > 0 ? fmt(profit / orders) : "0"}/order`}
        ...
        <KpiCard
          ...
          sub={adSpend > 0 ? `€${fmt(adSpend)} ad spend` : "Ads not connected"}
          ...
          <div className="flex items-center justify-between mb-2">
            ...
            {storeId !== "all" && !loading && (
              <button onClick={() => { setSpendEditing(true); setSpendInput(""); }}
                className="text-[10px] text-blue-400 hover:text-blue-600 font-medium">✎ override</button>
            )}
          </div>
          {loading ? <div className="h-6 w-20 rounded-lg bg-gray-100 animate-pulse" /> : spendEditing ? (
            <div className="flex gap-1 items-center">
              ...
              <input autoFocus type="text" value={spendInput} onChange={e => setSpendInput(e.target.value)}
                ...
                placeholder="bv. 4830" className="w-24 text-sm border border-blue-300 rounded px-1.5 py-0.5 outline-none" />
              <button onClick={saveSpend} disabled={spendSaving}
                className="text-xs bg-blue-500 text-white px-2 py-0.5 rounded font-semibold disabled:opacity-50">
                {spendSaving ? "…" : "OK"}
              </button>
              <button onClick={() => setSpendEditing(false)} className="text-xs text-gray-400">✕</button>
            </div>
          ) : (
            <p className="text-xl font-black text-gray-900">{adSpend > 0 ? `€${fmt(adSpend)}` : "—"}</p>
          )}
          <p className="text-[10px] text-gray-400 mt-1">{cpc > 0 ? `CPC €${cpc.toFixed(2)} · CTR ${ctr.toFixed(2)}%` : "No data"}</p>
        </div>
        <StatCard
          label="Returns"
      ...
      {/* ── Scaling Insights ───────────────────────────────────── */}
      {storeId !== "all" && <AiTips ov={ov} products={products} />}
    </div>
  );
}
