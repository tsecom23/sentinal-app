        ...
        <div className="flex items-center gap-1.5 mb-6">
          ...
          {hasGoogleData && (
            ...
            >
              ...
              Google <span className="opacity-70">€{fmt(googleAdSpend)}</span>
            </button>
          )}
          {hasMetaData && (
            <button
            ...
            >
              ...
              Meta <span className="opacity-70">€{fmt(metaAdSpend)}</span>
            </button>
          )}
          {adChannel !== "combined" && (
            <span className="ml-2 text-[10px] text-gray-400">
              {adChannel === "google"
                ? googleRevenue > 0 ? `€${fmt(googleRevenue)} attributed via UTM` : "No UTM data yet — sync orders to attribute revenue"
                : facebookRevenue > 0 ? `€${fmt(facebookRevenue)} attributed via UTM` : "No UTM data yet — sync orders to attribute revenue"}
            </span>
          )}
        </div>
      )}
