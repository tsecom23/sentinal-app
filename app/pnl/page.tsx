    ...
    <div className="p-6 space-y-5 min-h-screen">
      ...
      <div className="flex items-start justify-between flex-wrap gap-3">
        <div>
          ...
          <p className="text-gray-500 text-sm mt-0.5">Monthly overview of revenue, costs and profit</p>
        </div>
        <div className="flex items-center gap-2 flex-wrap">
          <div className="flex gap-1 bg-black/5 rounded-xl p-1">
            {[2024, 2025, 2026].map(y => (
              <button key={y} onClick={() => setYear(y)}
                className={`px-3 py-1.5 rounded-lg text-xs font-medium transition-all ${year === y ? "bg-emerald-600 text-gray-900" : "text-gray-500 hover:text-gray-900"}`}>
                {y}
              </button>
            ))}
          </div>
      ...
      <div className="grid grid-cols-2 md:grid-cols-5 gap-3">
        {[
          ...
          { label: "Net Profit",    value: totals ? fmt(totals.profit)        : "—",
            color: !totals ? "text-gray-500" : totals.profit >= 0 ? "text-emerald-400" : "text-red-400",
            sub: totals ? `${totalMargin}% margin` : "Margin %",
            highlight: totals ? (totals.profit >= 0 ? "bg-emerald-50 border-emerald-500/20" : "bg-red-50 border-red-500/20") : "" },
        ].map(card => (
          <div key={card.label} className={`border rounded-2xl p-4 ${card.highlight ?? "bg-black/3 border-black/5"}`}>
      ...
      <div className="flex gap-1 bg-black/5 rounded-xl p-1 w-fit">
        <button onClick={() => setTab("overview")}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${tab === "overview" ? "bg-black/10 text-gray-900" : "text-gray-500 hover:text-gray-600"}`}>
          <TrendingUp size={13} /> Monthly overview
        </button>
        <button onClick={() => setTab("costs")}
          className={`flex items-center gap-2 px-4 py-2 rounded-lg text-xs font-semibold transition-all ${tab === "costs" ? "bg-black/10 text-gray-900" : "text-gray-500 hover:text-gray-600"}`}>
          <Receipt size={13} /> Fixed Costs
          {costs.length > 0 && (
            <span className="bg-purple-500/30 text-purple-700 text-[10px] rounded-full px-1.5 font-bold">{costs.length}</span>
          )}
        </button>
      </div>
      ...
      {tab === "overview" && (
        <div className="space-y-4">
          ...
          <div className="bg-black/3 border border-black/5 rounded-2xl p-4">
            <p className="text-xs font-semibold text-gray-500 mb-3 uppercase tracking-widest">Profit trend {year}</p>
            {loading ? (
          ...
          <div className="bg-black/3 border border-black/5 rounded-2xl overflow-hidden">
            {/* Column headers */}
            <div className="grid grid-cols-[2fr_1fr_1fr_1fr_1fr] px-4 py-2.5 border-b border-black/5">
              {["Month", "Orders", "Net Revenue", "Profit", "Margin"].map((h, i) => (
                <span key={h} className={`text-[10px] text-gray-400 uppercase tracking-widest font-semibold ${i > 0 ? "text-right" : ""}`}>{h}</span>
              ))}
            </div>
                    ...
                    <div key={m.month}>
                      <button
                        ...
                        className={`w-full grid grid-cols-[2fr_1fr_1fr_1fr_1fr] px-4 py-3 transition-all ${
                          isCurrent ? "bg-blue-50 border-l-2 border-l-blue-500" : "border-l-2 border-l-transparent"
                        } ${active ? "hover:bg-black/3 cursor-pointer" : "cursor-default"}`}
                      >
                        {/* Month name */}
                        <div className="flex items-center gap-2">
                          {active
                            ? <ChevronDown size={12} className={`text-gray-500 shrink-0 transition-transform duration-150 ${isOpen ? "" : "-rotate-90"}`} />
                            : <span className="w-3 shrink-0" />
                          }
                          <span className={`text-sm font-semibold ${isCurrent ? "text-blue-700" : active ? "text-gray-700" : "text-gray-400"}`}>
                            {MONTH_NAMES[i]}
                          </span>
                          {isCurrent && <span className="text-[9px] bg-blue-500/20 text-blue-400 rounded px-1.5 py-0.5 font-bold">NOW</span>}
                        </div>
                        {/* Orders */}
                        <span className={`text-sm text-right tabular-nums ${active ? "text-gray-600" : "text-gray-400"}`}>
                          {active ? m.orders : "—"}
                        </span>
                        {/* Net revenue */}
                        <span className={`text-sm text-right tabular-nums font-medium ${active ? "text-gray-700" : "text-gray-400"}`}>
                          {active ? fmt(m.net_revenue) : "—"}
                        </span>
                        {/* Profit */}
                        <span className={`text-sm text-right tabular-nums font-bold ${!active ? "text-gray-400" : m.profit >= 0 ? "text-emerald-400" : "text-red-400"}`}>
                          {active ? fmt(m.profit) : "—"}
                        </span>
                        {/* Margin */}
                        <span className={`text-sm text-right tabular-nums ${!active ? "text-gray-400" : m.margin >= 0 ? "text-emerald-500" : "text-red-400"}`}>
                          {active ? `${m.margin}%` : "—"}
                        </span>
                      </button>
                      ...
                      {/* Expanded breakdown */}
                      {isOpen && active && (
                        <div className="mx-4 mb-2 rounded-xl bg-black/20 border border-black/5 overflow-hidden">
                          {[
                            { label: "Gross Revenue",    value: fmt(m.gross_revenue),                                 color: "text-gray-600" },
                            ...
                            <div key={row.label} className={`flex items-center justify-between px-4 py-2 ${ri < 4 ? "border-b border-black/5" : ""}`}>
            ...
            {/* Totals */}
            {totals && !loading && (
              <div className="border-t border-black/10 grid grid-cols-[2fr_1fr_1fr_1fr_1fr] px-4 py-3 bg-gray-50">
                <span className="text-xs font-bold text-gray-600 pl-5">Total {year}</span>
                <span className="text-xs font-bold text-gray-600 text-right tabular-nums">{totals.orders}</span>
                <span className="text-xs font-bold text-gray-700 text-right tabular-nums">{fmt(totals.net_revenue)}</span>
      ...
      {tab === "costs" && (
        <div className="bg-black/3 border border-black/5 rounded-2xl overflow-hidden">
          <div className="flex items-center justify-between px-5 py-4 border-b border-black/5">
            <div>
              <p className="text-sm font-bold text-gray-900">Fixed Costs</p>
              <p className="text-xs text-gray-500 mt-0.5">Abonnementen, salarissen en overige vaste lasten</p>
            </div>
            <button
              onClick={() => { setShowAdd(true); setEditId(null); }}
              className="flex items-center gap-2 bg-blue-600 hover:bg-blue-500 text-gray-900 text-xs font-semibold px-3 py-2 rounded-lg transition-all"
            >
          ...
          {/* Add form */}
          {showAdd && (
            <div className="px-5 py-4 border-b border-black/5 bg-blue-50">
              <p className="text-xs font-semibold text-blue-700 mb-3">Nieuwe kost</p>
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 mb-3">
                <input
                  className="bg-black/5 border border-black/10 rounded-xl px-3 py-2 text-sm text-gray-900 placeholder-zinc-600 focus:outline-none focus:border-blue-500/50"
                  placeholder="Name (e.g. Shopify)"
                  value={draft.name}
                  onChange={e => setDraft(p => ({ ...p, name: e.target.value }))}
                />
                <select
                  className="bg-black/5 border border-black/10 rounded-xl px-3 py-2 text-sm text-gray-600 focus:outline-none focus:border-blue-500/50"
              ...
              <div className="flex gap-2">
                <button onClick={addCost} disabled={saving || !draft.name || !draft.amount}
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-gray-900 text-xs font-semibold rounded-lg transition-all">
                  {saving ? "Saving…" : "Add"}
                </button>
                <button onClick={() => { setShowAdd(false); setDraft(EMPTY); }}
                  className="px-4 py-2 text-gray-500 hover:text-gray-600 text-xs rounded-lg transition-all">
                  Cancel
                </button>
              </div>
            </div>
          ...
          {cLoading ? (
            ...
            <div className="p-10 text-center space-y-2">
              <Receipt size={32} className="text-gray-400 mx-auto" />
              <p className="text-gray-500 text-sm">No fixed costs yet</p>
              <p className="text-gray-400 text-xs">Click &quot;Add cost&quot; to get started</p>
            </div>
          ) : (
            <>
              {/* Column headers */}
              <div className="grid grid-cols-[2fr_1fr_1fr_1fr_80px] px-5 py-2 border-b border-black/5">
                {["Name","Category","Amount","Frequency",""].map((h, i) => (
                  <span key={i} className="text-[10px] text-gray-400 uppercase tracking-widest font-semibold">{h}</span>
                ))}
              </div>
              <div className="divide-y divide-white/5">
                {costs.map(c => (
                    ...
                    {editId === c.id ? (
                      ...
                      <div className="space-y-2">
                        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-2">
                          <input className="bg-black/5 border border-black/10 rounded-lg px-3 py-1.5 text-xs text-gray-900 focus:outline-none"
                            value={editDraft.name} onChange={e => setEditDraft(p => ({ ...p, name: e.target.value }))} />
                          <select className="bg-black/5 border border-black/10 rounded-lg px-3 py-1.5 text-xs text-gray-600 focus:outline-none"
                            value={editDraft.category} onChange={e => setEditDraft(p => ({ ...p, category: e.target.value }))}>
                            {CATEGORIES.map(cat => <option key={cat.value} value={cat.value}>{cat.label}</option>)}
                          </select>
                          <input type="number" className="bg-black/5 border border-black/10 rounded-lg px-3 py-1.5 text-xs text-gray-900 focus:outline-none"
                        ...
                        <div className="flex gap-2">
                          <button onClick={() => saveCost(c.id)} disabled={saving}
                            className="flex items-center gap-1 px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-gray-900 text-xs font-semibold rounded-lg transition-all">
                            <Check size={11} /> {saving ? "Saving…" : "Save"}
                          </button>
                          <button onClick={() => setEditId(null)}
                            className="flex items-center gap-1 px-3 py-1.5 text-gray-500 hover:text-gray-600 text-xs rounded-lg transition-all">
                            <X size={11} /> Cancel
                          </button>
                        </div>
                      </div>
                      ...
                      <div className="grid grid-cols-[2fr_1fr_1fr_1fr_80px] items-center">
                        ...
                        <span className="text-xs text-gray-500 bg-black/5 rounded-lg px-2 py-1 w-fit">
                        ...
                        <div className="text-xs text-gray-500">
                          ...
                          {c.recurring === "yearly" && (
                            <span className="block text-gray-400">= {fmt(c.amount / 12)}/mo</span>
                          )}
                        </div>
                        <div className="flex items-center gap-1 justify-end">
                          <button onClick={() => startEdit(c)}
                            className="p-1.5 rounded-lg text-gray-400 hover:text-gray-600 hover:bg-black/5 transition-all">
                            <Pencil size={13} />
                          </button>
              ...
              {/* Totals */}
              <div className="border-t border-black/10 px-5 py-3 flex items-center justify-between bg-gray-50">
                <span className="text-xs text-gray-500">{costs.length} kost{costs.length !== 1 ? "en" : ""}</span>
