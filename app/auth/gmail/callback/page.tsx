...
function GmailCallbackInner() {
  ...
  useEffect(() => {
    ...
    if (error) {
      setStatus("error");
      setMessage("Gmail authorisation was denied. You can close this tab and try again.");
      return;
    }
    if (!code) {
      setStatus("error");
      setMessage("No authorisation code received from Google.");
      return;
    }
    ...
    fetch(`${API}/api/inbox/oauth/callback`, {
      ...
      body: JSON.stringify({ provider: "gmail", code, store_id: storeId }),
    })
      .then(r => r.json())
      .then((d: { ok: boolean; email?: string; error?: string }) => {
        if (d.ok) {
          setStatus("ok");
          setMessage(`Gmail connected (${d.email}). Redirecting…`);
          setTimeout(() => { window.location.href = "/inbox"; }, 1200);
        } else {
          setStatus("error");
          setMessage(d.error ?? "Failed to connect Gmail.");
        }
      })
      .catch(() => {
        setStatus("error");
